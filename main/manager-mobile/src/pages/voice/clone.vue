    ...
    <div class="audio-editor">
      <div style="display: flex; gap: 20rpx;">
        ...
        <div style="flex:1; display: flex; flex-direction: column; align-items: start; flex-wrap: wrap;">
          <label style="font-weight: 500; padding: 5rpx 0;">录音时请朗读以下文本：</label>
          <label style="text-align: left; color: red;">早上好！ 小优，今天天气怎么样？<br/>有点冷啊~空调调低两度。<br/>下午想听点音乐——播放周杰伦的《晴天》吧。<br/>对了晚上八点记得提醒我健身哦！</label>
        </div>
      </div>
      ...
      <div class="clone-btn">
        <button :disabled="!croppedAudioUrl || isCloning" @click="handleSubmit">开始复刻</button>
      </div>
  </div>
</template>
    ...
    methods: {
      ...
      async handleSubmit() {
        ...
        try {
          this.isCloning = true;
          // 从Blob URL获取Blob对象
          const response = await fetch(this.croppedAudioUrl);
          ...
          formData.append('name', '复刻音频-'+randomNum(1000,9999));

          API.cloneVoice(formData, ({data}) => {
            uni.hideLoading()
            if (data.code === 0) {
              uni.showToast({
                icon: 'none',

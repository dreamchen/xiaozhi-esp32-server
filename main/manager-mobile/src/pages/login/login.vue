	...
	<view class="login">
		<view class="content">
			...
			<view class="footer">
				...
				<template v-if="allowUserRegister">
					<text>|</text>
	...
	export default {
		data() {
			return {
				...
				isFocus: true ,// 是否聚焦
				allowUserRegister: getApp().globalData.appInfo.allowUserRegister,// 是否允许注册
			};
		},
		components:{
			wInput,
		...
		methods: {
			...
			fetchCaptcha() {
				...
				API.getCaptcha(this.captchaUuid, (res) => {
				  if (res.statusCode === 200) {
					const blob = new Blob([res.data], { type: 'image/jpeg' });
					this.captchaUrl = URL.createObjectURL(blob);
				  } else {
					uni.showToast({icon: 'none',title:'验证码加载失败，点击刷新'});
				  }
				});
			},
		}

		...
		<view class="content">
			...
			<view class="main">
				<view style="display: flex;justify-content: space-between;align-items: center;">
					<picker mode="selector" v-model:value="index" :range="array" range-key="label" style="position: absolute;" @change="bindPickerChange">
						<view class="uni-input picker">{{array[index].label}}<i class="fas fa-angle-down"></i></view>
					</picker>
					<wInput
						...
						maxlength="11"
						placeholder="手机号"
						style="flex:1; padding-left: 142px;"
					></wInput>
				</view>
				<view style="display: flex;justify-content: space-between;align-items: center; gap: 10px;">
					...
					<img loading="lazy" v-if="captchaUrl" :src="captchaUrl" alt="验证码"
									style="width: 100px; height: 30px; cursor: pointer;" @click="fetchCaptcha" />
				</view>
				<wInput
					v-model:value="form.mobileCaptcha"
					type="number"
					maxlength="6"
					placeholder="手机验证码"
...
<script>
	...
	import { getUUID, validateMobile } from '@/utils/index';
	...
	export default {
		data() {
			return {
				...
				captchaUrl: '',
				index: 0,
				array: getApp().globalData.appInfo.mobileAreaList || [],
				//logo图片 base64
		...
		methods: {
			bindPickerChange: function(e) {
				...
				this.form.areaCode = this.areaCodeList[this.index].key;
			},
			isShowAgree(){
				//是否选择协议
			...
			getVerCode(){
				...
				API.sendSmsVerification({
				  ...
				  if (res.statusCode === 200) {
					if(res.data.code===0){
						_this.$refs.runCode.runCode(this.form.mobileCaptcha);
						uni.showToast({
							icon: 'none',
							position: 'bottom',
							title: '验证码发送成功'
						...
						this.form.captcha = '';
						this.fetchCaptcha();
						uni.showToast({icon: 'none', position: 'bottom', title:res.data.msg});
					}
				  } else {
        			this.fetchCaptcha();
					uni.showToast({icon: 'none', position: 'bottom', title:'验证码发送失败，请稍后重试'});
				  }
				});
		    ...
		    startReg() {
				...
				if (this.form.mobileCaptcha.length != 6) {
				    uni.showToast({
				        icon: 'none',
						position: 'bottom',
				        title: '手机验证码不正确'
				...
				API.register(this.form, (res) => {
				  ...
				  if (res.statusCode === 200) {
					...
					uni.showToast({icon: 'none', position: 'bottom', title:'注册失败，请稍后重试'});
				  }
				})
		    },
			fetchCaptcha() {
      			this.form.captchaId = getUUID();
				API.getCaptcha(this.form.captchaId, (res) => {
				  if (res.statusCode === 200) {
					const blob = new Blob([res.data], { type: 'image/jpeg' });
					this.captchaUrl = URL.createObjectURL(blob);
				  } else {
					uni.showToast({icon: 'none', position: 'bottom', title:'验证码加载失败，点击刷新'});
				  }
				});
			},
		}

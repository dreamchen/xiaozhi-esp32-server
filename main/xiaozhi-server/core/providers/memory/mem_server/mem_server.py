...
class MemoryProvider(MemoryProviderBase):
    def __init__(self, config, read_config_from_api):
        logger.bind(tag=TAG).info(f"server memory __init__ config:{config}, read_config_from_api:{read_config_from_api}")
    ...
    def init_memory(self, role_id, llm):
        logger.bind(tag=TAG).info(f"server memory init_memory")
        super().init_memory(role_id, llm)
        #self.load_memory()

    def load_memory(self):
        logger.bind(tag=TAG).info(f"server memory load_memory")
        all_memory = {}
    ...
    async def save_memory(self, msgs):
        ...
        print(f"save_memory read_config_from_api: {self.read_config_from_api}")
        if not self.read_config_from_api:
            ...
            try:
                ...
                logger.bind(tag=TAG).info(f"save_memory, msgJson:{msgJson}")

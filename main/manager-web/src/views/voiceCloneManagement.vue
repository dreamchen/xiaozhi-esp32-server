<template>
    <div class="welcome">
        ...
        <div class="operation-bar">
            <h2 class="page-title">声音复刻</h2>
            <div class="right-operations">
                <el-input placeholder="请输入声音名称查询" v-model="searchName" class="search-input"
                ...
                <div class="content-area">
                    <el-card class="params-card" shadow="never">
                        <el-table ref="paramsTable" :data="paramsList" class="transparent-table" v-loading="loading"
                            ...
                            <el-table-column label="声音编码" prop="ttsVoice" align="center"></el-table-column>
                            <el-table-column label="声音名称" prop="name" align="center">      
                                <template slot-scope="scope">
                                    <el-input :ref="'voiceName_input_'+scope.$index" v-if="scope.row.isEdit" v-model="scope.row.name" size="small" class="input-pd0" @blur="stopEditVoiceName(scope.$index, scope.row)"></el-input>
                                    <span v-else>
                                        {{ scope.row.name }}
                                    </span>
        ...
        <!-- 新增/编辑声音对话框 -->
        <voice-clone-dialog :title="dialogTitle" :visible.sync="dialogVisible" @submit="handleSubmit"
...
export default {
    ...
    methods: {
        ...
        fetchVoiceCloneList() {
            ...
            Api.timbre.getCloneList(params, (res) => {
                this.loading = false;
                res = res.data
                if (res.code === 0) {
                    ...
                    this.paramsList = this.voiceCloneList;
        ...
        showAddDialog() {
            this.dialogTitle = "新增声音";
            this.$nextTick(() => {
                // 重置表单的校验状态
                if (this.$refs.voiceCloneDialog && this.$refs.voiceCloneDialog.$refs.form) {
                    this.$refs.voiceCloneDialog.$refs.form.clearValidate();
                }
            });
        ...
        deleteParam(row) {
            ...
            this.$confirm(`确定要删除选中的${paramCount}个声音吗？`, '警告', {
                ...
                Api.timbre.deleteCloneVoice(ids, (res) => {
                    res = res.data;
                    if (res.code === 0) {
                        this.$message.success({
                            message: `成功删除${paramCount}个声音`,
                            showClose: true
                        });
                        this.fetchVoiceCloneList();
                    } else {
        ...
        formatDate,
        startEditVoiceName(index, row) {
            this.oldVal = row.name;
            this.voiceCloneList[index].isEdit = true;
            this.$nextTick(()=>{
                this.$refs['voiceName_input_'+index].focus();
            })
        },
        stopEditVoiceName(index, row) {
            this.voiceCloneList[index].isEdit = false;
            if(this.oldVal === row.name) return;
            this.handleSetVoiceName(row.id, row.name);
        },
        handleSetVoiceName(id, name) {
            // 设置名称逻辑
            Api.timbre.updateCloneVoice({id, name}, ({data}) => {
                if (data.code === 0) {
                showSuccess('更新成功')
                this.fetchVoiceCloneList()
                } else {

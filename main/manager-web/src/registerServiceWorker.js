...
export const register = () => {
  if (process.env.NODE_ENV === 'production' && 'serviceWorker' in navigator) {
    window.addEventListener('load', () => {
      ...
      console.info(`[小优服务] 正在尝试注册Service Worker，URL: ${swUrl}`);
      
      // 先检查Service Worker是否已注册
      navigator.serviceWorker.getRegistrations().then(registrations => {
        if (registrations.length > 0) {
          console.info('[小优服务] 发现已有Service Worker注册，正在检查更新');
        }
        ...
        navigator.serviceWorker
          .register(swUrl)
          .then(registration => {
            console.info('[小优服务] Service Worker注册成功');
          ...
          .catch(error => {
            ...
            if (error.name === 'TypeError' && error.message.includes('Failed to register a ServiceWorker')) {
              console.warn('[小优服务] 注册Service Worker时出现网络错误，CDN资源可能无法缓存');

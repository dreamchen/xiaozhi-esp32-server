      ...
      if ('serviceWorker' in navigator) {
        try {
          ...
          if (registrations.length > 0) {
            ...
            setTimeout(async () => {
              ...
              if (!hasCaches) {
                ...
                if (process.env.NODE_ENV === 'development') {
                  console.info(
                    '%c[小优服务] 在开发环境中，Service Worker可能无法正常初始化缓存',
                    'color: #E6A23C; font-weight: bold;'
                  );
            ...
            console.info(
              '%c[小优服务] Service Worker未注册，CDN资源可能无法缓存', 
              'color: #F56C6C; font-weight: bold;'
            );
            
            if (process.env.NODE_ENV === 'development') {
              console.info(
                '%c[小优服务] 在开发环境中，这是正常现象',
                'color: #E6A23C; font-weight: bold;'
              );
